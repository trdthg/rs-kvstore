...
pub fn decode(bytes: &String) -> io::Result<Vec<u8>> {
...
fn original<T: Alphabet>(alphabet: &T, chunk: &[char]) -> Vec<u8> {
    chunk.iter()
        .filter(|char| *char != &alphabet.get_padding_char())
        .map(|char| alphabet.get_index_from_char(*char).expect("unable to find character from alphabet"))
        .collect()
}
